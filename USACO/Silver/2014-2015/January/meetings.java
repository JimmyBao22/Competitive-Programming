...
/*	
 	VV also kinda works, gets 12/13 (one gets runtime error)
	...
	for (int i=0; i<n; i++) {
		...
		else goingleft.add(i);
	}	
 	
 	for (int i=0; i<goingright.size(); i++) {
			min = 0;
			max = goingleft.size();
			int needed = arr[goingright.get(i)].pos + time;
			while (min < max) {
				int middle = (min + max+1)/2;
				if (middle == goingleft.size()) break;
				if (arr[goingleft.get(middle)].pos < arr[goingright.get(i)].pos) {
					min = middle+1;
				}
				else if (arr[goingleft.get(middle)].pos >= arr[goingright.get(i)].pos && 
						arr[goingleft.get(middle)].pos - time <= needed) {
					min = middle;
				}
				else max = middle-1;
			}
			int maxworks = min;
			if (arr[goingleft.get(min)].pos < arr[goingright.get(i)].pos ||
				!(arr[goingleft.get(min)].pos >= arr[goingright.get(i)].pos && 
						arr[goingleft.get(min)].pos - time <= needed)) {
				continue;
			}
			
			min = -1;
			max = goingleft.size()-1;
			while (min < max) {
				int middle = (min + max+1)/2;
				if (middle <= -1) break;
				if (arr[goingleft.get(middle)].pos < arr[goingright.get(i)].pos) {
