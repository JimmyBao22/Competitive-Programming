...
public class milkvisits {
	...
	public static void main(String[] args) throws IOException {
		...
		HashMap<Integer, ArrayList<Integer>> type = new HashMap<>();
			// type, index
		...
		for (int i=0; i<n; i++) {
			...
			if (!type.containsKey(t)) type.put(t, new ArrayList<>());
			type.get(t).add(i);
		}
		...
		for (int i=0; i<m; i++) {
			...
			q[i] = new B(i, one, two, c);
		}
		
		Arrays.sort(q);
		int[] ans = new int[m];
		for (int i=0; i<m; i++) {
			int current_type = q[i].type;
			
			if (!type.containsKey(current_type)) continue;

			for (Integer a : type.get(current_type)) {
				s.set(arr[a].stpos, current_type);
			}
			
			for (; i<m; i++) {
				if (q[i].type == current_type) {
					if (query(q[i].from, q[i].to) == current_type) ans[q[i].index] = 1;
				}
				else break;
			}
			i--;
