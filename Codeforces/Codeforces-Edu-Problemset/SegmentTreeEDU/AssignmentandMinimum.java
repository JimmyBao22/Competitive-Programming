...
public class AssignmentandMinimum {
	...
	public static void main(String[] args) throws IOException, FileNotFoundException {
		...
		//BufferedReader in = new BufferedReader(new FileReader("AssignmentandMinimum"));
	...
	static class SegTree {
		...
		public long comp(int l, int r, int x, int lx, int rx) {
			...
			propogate(x, lx, rx);
			if (l <= lx && rx <= r) return min[x];	// inside whole segment
			int m = (lx + rx)/2;			
		...
		public void print() {
			...
			System.out.println();
			for (int i=0; i<min.length; i++) System.out.print(min[i] + " ");
			System.out.println();
		}
	}
}
